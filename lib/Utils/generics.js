...
const generateMessageIDV2 = (userId) => {
    ...
    // Sisipkan kata F4YKAL ke posisi acak dalam partialHash
...
const generateMessageID = () => {
    ...
    // Mengacak posisi untuk menempatkan kata "F4YKAL"
    const randomPos = Math.floor(Math.random() * (length - wordLength + 1));
    // Membuat bagian sebelum kata "F4YKAL"
    let id = "";
    for (let i = 0; i < randomPos; i++) {
        id += chars.charAt(Math.floor(Math.random() * chars.length));
    }
    // Menambahkan kata "F4YKAL"
    id += word;
    // Membuat bagian setelah kata "F4YKAL"
